...
loginBtn?.addEventListener('click', async () => {
    ...
    try {
        const res = await fetch('http://18.205.26.84:3000/proxy/access/login', {
            method: 'POST',
...
async function checkSession() {
    try {
        const res = await fetch('http://18.205.26.84:8080/api/verified', {
            method: 'POST',
            credentials: 'include'
        });
...
async function loadProfileButton() {
    ...
    try {
        const response = await fetch(`http://18.205.26.84:8080/api/users/${userId}`, {
            method: 'GET',
            credentials: 'include'
        });
