...
loginBtn?.addEventListener('click', async () => {
    ...
    try {
        const res = await fetch('http://18.235.28.236:3000/proxy/access/login', {
            method: 'POST',
...
async function checkSession() {
    try {
        const res = await fetch('http://18.235.28.236:8080/api/verified', {
            method: 'POST',
            credentials: 'include'
        });
...
async function loadProfileButton() {
    ...
    try {
        const response = await fetch(`http://18.235.28.236:8080/api/users/${userId}`, {
            method: 'GET',
            credentials: 'include'
        });
